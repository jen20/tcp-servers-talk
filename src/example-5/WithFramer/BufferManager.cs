...
{
    /// <summary>
    /// A manager to handle buffers for socket connections
    /// </summary>
    /// <remarks>
    ...
    /// segments that they can use for their buffers.
    /// 
    /// The alternative to this would be to create many small arrays which it then
    /// maintained. This methodology should be slightly better than the many small array
    /// methodology because in creating only a few very large objects it will force these
    /// objects to be placed on the LOH.
    /// 
    /// Since the  objects are on the LOH they are at this time not subject to compacting
    /// which would  require an update of all GC roots as would be the case with lots of
    /// smaller arrays that were in the normal heap.
    /// </remarks>
    public class BufferManager
    {
        private const int MaxAllocationAttempts = 100;
        ...
        public ArraySegment<byte> CheckOut()
        {
            ...
            throw new UnableToAllocateBufferException(MaxAllocationAttempts);
        }
        ...
        public IEnumerable<ArraySegment<byte>> CheckOut(int toGet)
        {
            ...
            while (count < MaxAllocationAttempts)
            {
        ...
        /// <remarks>
        /// It is the client's responsibility to return the buffer to the manger by
        /// calling <see cref="CheckIn(System.ArraySegment{byte})"></see> with the buffer
        /// </remarks>
